...
import { EventEntriesPlayerGrid, EventEntriesTablePlayer, EventEntriesTableTeam, EventEntriesTeamGrid } from "#components"
            ...
            players: [
              {
                ...
                last_name: entry.last_name,
                country: entry.country,
                singles_rank: singlesEntries[0]?.rank
              }
            ]
          })
        } else {
            ...
            players: [
              {
                ...
                last_name: entry.last_name,
                country: entry.country,
                doubles_rank: doublesEntries[0]?.rank
              },
              partnerEntry ?
                {
                  id: partnerEntry.id,
                  ...
                  doubles_rank: partnerEntry.rank
                }
              : null
            ].filter(Boolean)
          })
...
<template>
  ...
  >
    ...
    <component
      ...
      :is="viewMode === 'list' ? EventEntriesTablePlayer : EventEntriesPlayerGrid"
      :key="`player-${viewMode}`"
    ...
    <component
      v-else
      :is="viewMode === 'list' ? EventEntriesTableTeam : EventEntriesTeamGrid"
      :key="`team-${viewMode}`"
      :entries="consolidatedEntries"
      :status="status"
    />
