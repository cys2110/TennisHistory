...
const { h2h } = defineProps<{ h2h: { p1: H2HPlayerType; p2: H2HPlayerType } }>()

const p1Numbers = computed(() => {
  return {
    wl: percentage(Number(h2h.p1.wins), Number(h2h.p1.wins) + Number(h2h.p1.losses)),
    titles: Number(h2h.p1.titles),
    pm: Number(h2h.p1.pm.replaceAll(",", "")),
    ch: h2h.p1.ch ? Number(h2h.p1.ch) : 0
  }
})

const p2Numbers = computed(() => {
  return {
    wl: percentage(Number(h2h.p2.wins), Number(h2h.p2.wins) + Number(h2h.p2.losses)),
    titles: Number(h2h.p2.titles),
    pm: Number(h2h.p2.pm.replaceAll(",", "")),
    ch: h2h.p2.ch ? Number(h2h.p2.ch) : 0
  }
})
...
const comparisons = computed(() => [
  {
    title: "Win-Loss",
    p1: `${h2h.p1.wins}—${h2h.p1.losses}`,
    p2: `${h2h.p2.wins}—${h2h.p2.losses}`,
