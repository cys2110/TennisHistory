...
import { BaseLink, CountryLink, EventButtons, TournamentLink, UButton } from "#components"
...
const columns: TableColumn<EventCardType>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return h(
        ...
        () => row.getValue("tournament.name")
      )
    },
    meta: {
      class: {
        td: "max-w-20 xl:max-w-fit whitespace-normal break-words"
      }
    }
  },
  {
    ...
    meta: {
      class: {
        td: "max-w-20 xl:max-w-fit whitespace-normal break-words md:text-xs lg:text-sm"
      }
    }
  },
  {
