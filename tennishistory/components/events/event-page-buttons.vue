...
const { name, tid, eid, year, drawType } = defineProps<{ name: string; tid: string; eid: string; year: string; drawType: DrawType }>()
...
<template>
  ...
  >
    <u-button
      ...
      :label="page.label"
      :icon="page.icon"
      :to="{ name: page.name, params: { name, tid, eid, year } }"
