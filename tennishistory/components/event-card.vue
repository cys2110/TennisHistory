...
<template>
	...
	>
		<shadcn-card-header>
			<shadcn-card-title class="flex items-center gap-3">
				...
				<div>
					<nuxt-link
						class="hover-link"
						:to="{ name: 'tournament', params: { tname: tournamentName, tid } }"
					>
						{{ tname }}
					</nuxt-link>
				</div>
		...
		<shadcn-card-footer class="flex flex-wrap justify-center md:justify-evenly mt-5 gap-2 lg:gap-1">
			<template
			...
			>
				<div
					v-if="isDisabled"
					v-tooltip="'Event has not started yet'"
				>
					<shadcn-button
						v-tooltip="'Event has not started yet'"
						disabled
