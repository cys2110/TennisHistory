export default defineEventHandler(async query => {
  ...
  const { records } = await useDriver().executeQuery(
    `/* cypher */
      ...
      WITH c, MIN(e.start_date) AS first_win_date, COUNT(DISTINCT p) AS wins, COUNT(p) AS total_wins
      ORDER BY wins DESC, first_win_date ASC
      RETURN toString(wins) AS wins, {id: c.id, name: c.name, alpha2: c.alpha2} AS country, toString(total_wins) AS total_wins
    `,
    { id: Number(id) }
  )
