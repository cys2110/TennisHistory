export default defineEventHandler(async query => {
  ...
  const { records } = await useDriver().executeQuery(
    ...
    OPTIONAL MATCH
      ...
      (y:Year)
    WHERE
      (e.category IN categories OR
        (p:ATP AND e.atp_category IN categories) OR
        (p:WTA AND e.wta_category IN categories)) AND
      ((type(z) = 'REPRESENTED' AND
          (z.start_date <= e.start_date OR
            (p:ATP AND
              z.start_date <= e.atp_start_date AND
              z.end_date >= e.atp_end_date) OR
            (p:WTA AND
              z.start_date <= e.wta_start_date AND
              z.end_date >= e.wta_end_date))) OR
        type(z) = 'REPRESENTS')
    OPTIONAL MATCH (e)-[:EDITION_OF]->(t:Tournament)
    OPTIONAL MATCH (p)-[:REPRESENTS]->(c1:Country)
    WITH
      p,
      properties(c1) AS country,
    ...
    WITH
      ...
      apoc.map.merge(
        apoc.map.submap(
          e,
          ['id', 'category', 'atp_category', 'wta_category'],
          null,
          false
        ),
        {tournament: properties(t), year: y.id, type: type}
      ) AS event
    RETURN player, COLLECT(DISTINCT event) AS events
    `,
    { id }
  )
