      ...
      <v-col cols="4" v-for="group in groupedTies" :key="group[0].round">
        <short-card class="mx-3 py-3">
          ...
          <v-card-subtitle class="text-center"
            >{{ group[0].venue }}, {{ group[0].city }}</v-card-subtitle
          >
          <div v-for="tie in group" :key="tie.id">
            <v-card-text>
              <div class="d-flex justify-space-evenly">
                <div>
                  <flag-img :src="flagSrc(tie.country1)" :alt="tie.country1" width="100" />
                </div>
                <div class="d-flex flex-nowrap align-center">
                  <span
                    class="text-h6"
                    :class="{ 'font-weight-bold': score(tie.MatchScores, tie.NonCounts) > 1 }"
                    >{{ score(tie.MatchScores, tie.NonCounts) }}</span
                  >
                  <span class="text-h6">-</span>
                  <span
                    class="text-h6"
                    :class="{ 'font-weight-bold': score(tie.MatchScores, tie.NonCounts) <= 1 }"
                    >{{ 3 - score(tie.MatchScores, tie.NonCounts) }}</span
                  >
                </div>
                <div>
                  <flag-img :src="flagSrc(tie.country2)" :alt="tie.country2" width="100" />
                </div>
              </div>
            </v-card-text>
            <v-card-actions>
              <v-chip class="mx-auto"
                ...
                <v-dialog activator="parent" transition="dialog-bottom-transition">
                  <TeamMatches
                    :match="tie.MatchScores[0]"
                    :wta="tie.NonCounts"
                    :country1="tie.country1"
                    :country2="tie.country2"
                  />
                </v-dialog>
              </v-chip>
            </v-card-actions>
