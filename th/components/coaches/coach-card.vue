...
defineProps<{ coach: { labels: string[]; id: string; name: string | null; country: CountryInterface | null } }>()
</script>
...
<template>
  <u-page-card
    ...
    :icon="coach.country?.alpha2 ? `flag:${coach.country?.alpha2}-4x3` : coach.country ? `flags:${coach.country?.id}` : undefined"
    highlight
