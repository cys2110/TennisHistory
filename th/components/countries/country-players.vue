...
// API call
const { data, status, refresh } = await useFetch<CountryPlayerAPIResponse>("/api/country-players", {
  query: { id, skip: computed(() => (page.value - 1) * pageSize.value), limit: pageSize.value },
  watch: false,
  onResponseError: ({ error }) => {
    toast.add({
      title: `Error fetching players who previously represented or currently represent ${name ?? id}`,
...
watch(
  ...
  ([newId, newPage, newPageSize]) => {
    if ((newId || newPage || newPageSize) && newId !== " " && route.name === "country") refresh()
  }
)
</script>
...
<template>
  ...
  >
    ...
    <error-message
      ...
      :status
      :error="`players who previously represented or currently represent ${name ?? id}`"
    />
  </dashboard-subpanel>
</template>
