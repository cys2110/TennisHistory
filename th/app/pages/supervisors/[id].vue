...
// API call
const { data: supervisor } = await useFetch<PersonInterface>("/api/supervisors/overview", {
  key: `supervisors-overview-${id}`,
  query: { id },
  onResponse: ({ response }) => (year.value = response._data?.years?.[response._data.years.length - 1] || new Date().getFullYear())
})

const { data: events, status } = await useFetch<EventInterface[]>("/api/supervisors/details", {
  key: `supervisors-details-${id}-${year}`,
  query: { id, year },
  default: () => [],
  server: false
})
...
useHead({
  title: () => `${supervisor.value?.id ?? capitalCase(id as string)} | Supervisors`
})
useJsonld(() => ({
  "@context": "https://schema.org",
  "@type": "Person",
  name: supervisor.value?.id ?? capitalCase(id as string),
  jobTitle: "Tournament supervisor"
}))
          ...
          <template #empty>
            <div class="flex justify-center items-center w-full gap-2 text-error">
              <u-icon
                ...
                class="text-base"
              />
              {{ supervisor ? `${supervisor.first_name} ${supervisor.last_name}` : capitalCase(id as string) }} did not supervised any events in
              {{ year }}.
            </div>
          </template>
        </u-table>
