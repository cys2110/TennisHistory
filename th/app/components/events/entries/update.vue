...
const state = reactive<Partial<EntrySchema>>({
  event: `${edId}-${tour}`,
  id: entry?.id,
  type,
  seed: entry?.seed,
...
const formFields: FormFieldInterface<EntrySchema>[] = [
      ...
      >
        <div class="grid grid-cols-2 gap-5 items-center">
          ...
          <form-field
            ...
            :field="{ label: 'Player 1', key: 'player1', type: 'search', subType: 'players' }"
          />
          <form-field
            v-model="state['rank']"
          ...
          />
          <form-field
            v-if="!entry && state.type === 'Doubles'"
            v-model="state['player2']"
            :field="{ label: 'Player 2', key: 'player2', type: 'search', subType: 'players' }"
          />
          <form-field
            v-if="!entry && state.type === 'Doubles'"
            v-model="state['rank2']"
            :field="{ label: 'Rank', key: 'rank2', type: 'number' }"
          />
        </div>
      </u-form>
    </template>
