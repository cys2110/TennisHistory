...
const selectedTour = ref<TourType>(tours.value?.[0] || "ATP")
...
const { data, status } = await useFetch<any>("/api/events/draws", {
  key: () => `draw-${eid}-${selectedTour.value}-${selectedType.value}-${selectedDraw.value}`,
  query: { id: eid, tour: selectedTour, type: selectedType, draw: selectedDraw },
  default: () => [],
  server: false
})
...
const formattedData = computed<DrawInterface>(() => {
  ...
  const qualifyingIndexMapping = {
    ...
    "Qualifying round 2": rounds.value.includes("Qualifying round 3") ? allData.filter((m: any) => m.round === "Qualifying round 2").length + 1 : 1,
    "Qualifying round 1": allData.filter((m: any) => m.round !== "Qualifying round 1").length + 1
  }

  for (const match of allData) {
    const team1Id =
      match.t1[0].first_name ?
        match.t1
          .map((p: any) => p.id)
          .sort()
          .join("-")
    ...
    const matchIndex =
      ["3rd Place Match", "Bronze Medal Match"].includes(match.round) ? 1
      : !match.round.includes("Qualifying") ? match.match.match_no - matchIndexMapping[match.round as keyof typeof matchIndexMapping]
    ...
    if (isDefined(match.s1?.s1)) {
      const s1Score: DrawScoreInterface = {
        mainScore: match.s1.s1
      }
      const s2Score: DrawScoreInterface = {
        mainScore: match.s2.s1
      }
      if (isDefined(match.s1.t1)) {
        s1Score.subscore = match.s1.t1 ?? match.s2.t1 + 2
        s2Score.subscore = match.s2.t1 ?? match.s1.t1 + 2
      }
      if (match.s1.s1 > match.s2.s1) {
        s1Score.isWinner = true
      } else if (match.s1.s1 < match.s2.s1) {
        s2Score.isWinner = true
      }
      side1Object.scores = [s1Score]
      side2Object.scores = [s2Score]
    }

    if (isDefined(match.s1?.s2)) {
      const s1Score: DrawScoreInterface = {
        mainScore: match.s1.s2
      }
      const s2Score: DrawScoreInterface = {
        mainScore: match.s2.s2
      }
      if (isDefined(match.s1.t2)) {
        s1Score.subscore = match.s1.t2 ?? match.s2.t2 + 2
        s2Score.subscore = match.s2.t2 ?? match.s1.t2 + 2
      }
      if (match.s1.s2 > match.s2.s2) {
        s1Score.isWinner = true
      } else if (match.s1.s2 < match.s2.s2) {
        s2Score.isWinner = true
      }
      side1Object.scores?.push(s1Score)
      side2Object.scores?.push(s2Score)
    }

    if (isDefined(match.s1?.s3)) {
      const s1Score: DrawScoreInterface = {
        mainScore: match.s1.s3
      }
      const s2Score: DrawScoreInterface = {
        mainScore: match.s2.s3
      }
      if (isDefined(match.s1.t3)) {
        s1Score.subscore = match.s1.t3 ?? match.s2.t3 + 2
        s2Score.subscore = match.s2.t3 ?? match.s1.t3 + 2
      }
      if (match.s1.s3 > match.s2.s3) {
        s1Score.isWinner = true
      } else if (match.s1.s3 < match.s2.s3) {
        s2Score.isWinner = true
      }
      side1Object.scores?.push(s1Score)
      side2Object.scores?.push(s2Score)
    ...
    if (isDefined(match.s1?.s4)) {
      const s1Score: DrawScoreInterface = {
        mainScore: match.s1.s4
      }
      const s2Score: DrawScoreInterface = {
        mainScore: match.s2.s4
      }
      if (isDefined(match.s1.t4)) {
        s1Score.subscore = match.s1.t4 ?? match.s2.t4 + 2
        s2Score.subscore = match.s2.t4 ?? match.s1.t4 + 2
      }
      if (match.s1.s4 > match.s2.s4) {
        s1Score.isWinner = true
      } else if (match.s1.s4 < match.s2.s4) {
        s2Score.isWinner = true
      }
      side1Object.scores?.push(s1Score)
      side2Object.scores?.push(s2Score)
    ...
    if (isDefined(match.s1?.s5)) {
      const s1Score: DrawScoreInterface = {
        mainScore: match.s1.s5
      }
      const s2Score: DrawScoreInterface = {
        mainScore: match.s2.s5
      }
      if (isDefined(match.s1.t5)) {
        s1Score.subscore = match.s1.t5 ?? match.s2.t5 + 2
        s2Score.subscore = match.s2.t5 ?? match.s1.t5 + 2
      }
      if (match.s1.s5 > match.s2.s5) {
        s1Score.isWinner = true
      } else if (match.s1.s5 < match.s2.s5) {
        s2Score.isWinner = true
      }
      side1Object.scores?.push(s1Score)
      side2Object.scores?.push(s2Score)
    ...
    if ((match.match.incomplete && match.match.incomplete !== "B") || match.s1?.incomplete || match.s2?.incomplete) {
      matchObject.matchStatus =
        match.match.incomplete ? INCOMPLETES[match.match.incomplete as keyof typeof INCOMPLETES]
        : match.s1?.incomplete ? INCOMPLETES[match.s1.incomplete as keyof typeof INCOMPLETES]
        : INCOMPLETES[match.s2.incomplete as keyof typeof INCOMPLETES]
    }

    matches.push(matchObject)
  }

  const lastRoundsCount =
    allData?.[0]?.round?.includes("Qualifying") ?
      Object.keys(contestants).length === 4 ?
        0
      : 2
    : 0
