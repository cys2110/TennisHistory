  ...
  const { records } = await useDriver().executeQuery(
    `/* cypher */
      ...
      WITH CASE
        ...
        ELSE NULL
      END AS events, t, p
      WITH {name: t.name, tid: toString(t.id), round: 'Win', events: events} AS wins, p, t
      ...
      WITH CASE
        ...
        WHEN SIZE(lowestRound.items) > 0 THEN {name: t.name, tid: toString(t.id), round: lowestRound.items[0].round, events: apoc.coll.flatten([x IN lowestRound.items | x.events])}
        ELSE NULL
      END AS events
