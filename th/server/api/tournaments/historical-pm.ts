export default defineEventHandler(async query => {
  ...
  const { records } = await useDriver().executeQuery(
    `/* cypher */
      ...
      WITH y.id AS year, e.currency AS currency, e.id AS id,
      ...
      WITH year, pm, currency, id,
          ...
          REDUCE(w = NULL, x IN rounds | CASE WHEN x.round = 'Win'      THEN x ELSE w END) AS win
      ...
      RETURN {
        ...
        W: CASE WHEN win IS NULL THEN null ELSE win.pm END
      } AS round_data
    `,
    { id: Number(id) }
  )
