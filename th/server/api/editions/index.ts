  ...
  const { records, summary } = await useDriver().executeQuery(
    ...
    CALL () {
      ...
      CALL (e) {
        ...
        WITH CASE WHEN f:Singles THEN 'Singles' ELSE 'Doubles' END AS type, COLLECT(DISTINCT apoc.map.merge(apoc.map.submap(p, ['id', 'first_name', 'last_name'], null, false), { country: playerCountry})) AS team, [x IN labels(event) WHERE x <> 'Event'][0] AS tour, winner_ids
        ORDER BY tour, type DESC
        RETURN COLLECT(DISTINCT{team: team, type: type, tour: tour}) AS winners, winner_ids
      }
      WITH e, y, s, venues, winners, winner_ids, [x IN labels(e) WHERE x <> 'Edition'] AS tours
