export default defineEventHandler(async query => {
    ...
    `/* cypher */
      ...
      CALL (p) {
        ...
        RETURN
          SUM(
            CASE
              WHEN s:Winner AND (m:Best3 OR m:Best5) THEN 1
              ELSE 0
            END) AS win,
          SUM(
            CASE
              WHEN s:Loser AND (m:Best3 OR m:Best5) THEN 1
              ELSE 0
            END) AS loss,
          SUM(
            CASE
      ...
      RETURN
        ...
        CASE
          ...
          ELSE null
        END AS ch_date,
        toString(win) AS wins,
        toString(loss) AS losses,
  ...
  return {
    ...
    turned_pro: player.pro ? Number(player.pro) : null,
    retired: player.retired ? Number(player.retired) : null,
    ch: player.ch ? Number(player.ch) : null,
    titles: Number(player.titles),
