export default defineEventHandler(async (query) => {
  ...
  const { records } = await useDriver().executeQuery(
    `/* cypher */
      ...
      CALL (p) {
        OPTIONAL MATCH (p)-[:ENTERED]->(:Entry)-[:SCORED]->(w:Winner)-[:SCORED]->(m:Best3 | Best5)
        OPTIONAL MATCH (p)-[:ENTERED]->(:Entry)-[:SCORED]->(l:Loser)-[:SCORED]->(m)
        WITH COUNT(DISTINCT(m)) AS win, COUNT(DISTINCT(l)) AS loss
        RETURN win, loss
      }
      RETURN {
        ch: toString(p.ch),
