...
export default defineEventHandler(async event => {
  interface QueryProps {
    ...
    reason: string
    teammate: string
    entry: SelectOptionsType
    players: SelectOptionsType[] | SelectOptionsType
  }
  ...
  switch (relationship) {
    ...
    case "Last Direct Acceptance":
      ...
      break
    case "Withdrawal":
      formattedParams["player1"] = Array.isArray(players) ? (players[0].value as string) : (players.value as string)
      formattedParams["player2"] = Array.isArray(players) && players.length > 1 ? (players[1].value as string) : null
      formattedParams["reason"] = reason
      formattedParams["teammate"] = teammate
      query = `/* cypher */
        CYPHER 25
        ...
        MATCH (e:Event {id: $eventId})
        MERGE (f:Entry:$($type) {id: $eventId || ' ' || $player1})
        MERGE (p1)-[:ENTERED]->(f)
        CALL (f) {
          WHEN $player2 IS NOT NULL THEN {
            MATCH (p2:Player {id: $player2})
            MERGE (p2)-[:ENTERED]->(f)
            SET f.id = f.id || ' ' || $player2
          }
        }
        CALL (*) {
