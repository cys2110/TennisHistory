export default defineEventHandler(async (event) => {
  ...
  const { records: statsRecords } = await useDriver().executeQuery(
    `/* cypher */
      MATCH (p:Player {id: $id})-[:ENTERED]->(f:Entry)-[:SCORED]->(s:Score)-[:SCORED]->(m:Match)-[:PLAYED]->(r:Round)-[:ROUND_OF]->(e:Event)-[:IN_YEAR]->(y:Year {id: $year})
  ...
  const statsResults = statsRecords.map((record) => record.toObject());
  const activityResults = activityRecords.map((record) => record.toObject());

  return {stats: statsResults[0].stats, activity: activityResults[0].events};
});
