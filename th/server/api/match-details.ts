export default defineEventHandler(async query => {
  ...
  const splitId = mid.split("")

  const matchId = {
    draw: mid.startsWith("m") ? "Main" : "Qualifying",
    tour: splitId[1] === "l" ? "WTA" : "ATP",
    itf: splitId[1] === "l" ? "Women" : "Men",
    type: splitId[2] === "d" ? "Doubles" : "Singles",
    number: splitId.slice(3).join("").trim()
  }

  const params = {
    id: Number(id),
    mid: Number(matchId.number),
    tour: matchId.tour,
    itf: matchId.itf,
    draw: matchId.draw,
    type: matchId.type
  ...
  const { records } = await useDriver().executeQuery(
    `/* cypher */
      MATCH (m:Best3|Best5 {match_no: $mid})-[:PLAYED]->(r:Round)-[:ROUND_OF]->(e:Event {id: $id})-[:ON_SURFACE]->(s:Surface)
      WHERE ($tour IN labels(m) OR $itf IN labels(m)) AND $draw IN labels(m) AND $type IN labels(m)
      ...
      CALL (s1, e) {
        ...
        RETURN {
          ...
          country: CASE
            WHEN c1 IS NOT NULL AND x.start_date <= e.start_date AND x.end_date > e.start_date THEN {id: c1.id, name: c1.name, alpha2: c1.alpha2}
  ...
  for (const [key, value] of Object.entries(match.match)) {
    if (Array.isArray(value)) {
      if (value.some(item => item === null)) {
        delete match.match[key]
      } else {
        match.match[key] = value.map(item => item.low)
      }
    }
  }
