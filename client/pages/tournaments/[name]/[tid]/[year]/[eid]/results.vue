...
const { data: results, status, error } = await useFetch<{ name: string; rounds: EventResultsType[] }>("/api/getResults", { query: { eid: route.params.eid } });

useHead({ title: `Results | ${results.value?.name ?? useChangeCase(route.params.name as string, "capitalCase").value} ${route.params.year}` });
</script>
	...
	<u-page>
		...
		<u-page-body>
			<u-alert
				...
				:icon="status === 'pending' ? 'i-line-md-loading-twotone-loop' : 'i-pepicons-print-grid-circle-off'"
