...
import { COLOURS, STATUS_INFO } from '@/utils/variables'
import { EventEntries, StatusInfo } from '@/utils/types'
...
const entryInfo = [
  ...
  { title: 'Status', value: status ? STATUS_INFO[status] : '—' },
]
...
onMounted(async () => {
  ...
  try {
    const flags = await import(`@/components/icons/flags`)
...
<template>
  <a-badge-ribbon :text="`Rank: ${rank || '—'}`" :color="COLOURS.green800">
    <a-card class="full-card pt-5 items-center" :bodyStyle="{ width: '100%' }">
      <template #cover>
        <a-image
          :alt="full_name"
          :src="headshot(id)"
          class="!rounded-full !border-zinc-300 border-[1.5px] !w-28"
          :preview="false"
        />
      </template>
      <a-card-meta class="text-center">
        <template #title>
          <router-link
            class="hover-link font-bold text-wrap"
