...
<template>
    <tr>
        <td class="flex items-center text-nowrap">
            <div class="mx-1">
                <flag-img v-if="player.node.country" class="w-[2rem]" :src="flag(player.node.country.id)"
                    :alt="player.node.country.name" />
            </div>
            <div class="mx-1">
                <v-avatar size="small">
                    <v-img :src="headshot(player.node.id)" :alt="player.node.full_name" />
                </v-avatar>
            </div>
            <div class="mx-1">
        ...
        <td class="text-zinc-300 text-xs text-right pr-10">
