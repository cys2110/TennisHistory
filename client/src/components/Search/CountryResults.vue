...
<template>
  ...
  <a-drawer
    ...
    class="!bg-violet-800"
  >
    <template #title>Players who have represented {{ selection }} </template>
    <a-list
      v-if="players.players.length > 0"
      :data-source="players.players"
      header="Current or retired players"
    >
      <template #renderItem="{ item }">
        <a-list-item>
          <a-list-item-meta>
    ...
    <a-list
      v-if="players.formerPlayers.length > 0"
      :data-source="players.formerPlayers"
      header="Former players"
    >
      <template #renderItem="{ item }">
        <a-list-item>
          <a-list-item-meta>
