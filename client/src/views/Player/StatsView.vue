...
// Handle selection
const handleSelection = () => router.push({ query: { year: selectedYear.value, surface: selectedSurface.value } })
...
watch(result, (newResult) => {
    if (newResult) {
        ...
        const rawBarStats = [
            { category: '1st serve', actualValue: playerStats.serve1, max: playerStats.serve1 + playerStats.serve2 },
            { category: '1st serve points won', actualValue: playerStats.serve1_w, max: playerStats.serve1 },
            { category: '2nd serve points won', actualValue: playerStats.serve2_w, max: playerStats.serve2 },
            { category: 'Break points won', actualValue: playerStats.bps_saved, max: playerStats.bps_faced },
            { category: 'Service points won', actualValue: playerStats.serve1_w + playerStats.serve2_w, max: playerStats.serve1 + playerStats.serve2 },
            { category: '1st serve return points won', actualValue: playerStats.ret1_w, max: playerStats.ret1 },
            { category: '2nd serve return points won', actualValue: playerStats.ret2_w, max: playerStats.ret2 },
            { category: 'Break points converted', actualValue: playerStats.bps_converted, max: playerStats.bp_opps },
            { category: 'Return points won', actualValue: playerStats.ret1_w + playerStats.ret2_w, max: playerStats.ret1 + playerStats.ret2 },
            { category: 'Total points won', actualValue: playerStats.serve1_w + playerStats.serve2_w + playerStats.ret1_w + playerStats.ret2_w, max: playerStats.serve1 + playerStats.serve2 + playerStats.ret1 + playerStats.ret2 }
        ]
        barStats.value = rawBarStats.map(stat => ({ ...stat, value: percentage(stat.actualValue, stat.max) }))
...
<template>
    <a-row>
        ...
        <a-col :span="3">
            <a-select :options="surfaces" class="w-[90%] text-center" size="large" v-model:value="selectedSurface"
                @change="handleSelection" />
        </a-col>
    </a-row>
    <a-row>
