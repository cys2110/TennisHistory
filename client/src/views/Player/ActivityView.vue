...
watch(
  () => props.name,
  () => updateDocumentTitle(`Activity | ${unencodeName(props.name)} | TennisHistory`),
  { immediate: true },
)
...
watch(
  ...
  (newResult) => {
    if (newResult) {
      ...
      stats.value = [
        {
          Wins: newResult.yearStats.wins,
          Losses: newResult.yearStats.losses,
          Titles: newResult.yearStats.titles,
        },
      ]
    ...
    <a-col :span="20" class="p-5">
      ...
      <a-space direction="vertical" size="large">
        <ActivityCard
          ...
          :key="event.scores[0].match.round.event.id"
          :event
          :name
          :id
        />
