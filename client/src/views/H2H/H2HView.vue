...
type H2HPlayer = Pick<Player, 'bh1' | 'career_high' | 'ch_date' | 'country' | 'dob' | 'full_name' | 'height_cm' | 'id' | 'loss' | 'pm_USD' | 'rh' | 'titles' | 'turned_pro' | 'win'> | null

const buttonOptions = {
  outlinedPrimaryColor: '{emerald.600}',
...
const p1Details: Ref<H2HPlayer> = ref(null)
const p1Url = ref('')
const p2Details: Ref<H2HPlayer> = ref(null)
const p2Url = ref('')
...
watch(() => [props.p1Name, props.p2Name], () => updateDocumentTitle(`${p1.value.name} v. ${p2.value.name}`), { immediate: true })
...
watch(result, (newResult) => {
  if (newResult) {
    ...
    pieStats.value = [
      {
        name: unencodeName(newResult.p1[0].full_name),
        value: newResult.p1Wins.count,
        itemStyle: { color: '#0e7490' },
      },
      {
        name: unencodeName(newResult.p2[0].full_name),
