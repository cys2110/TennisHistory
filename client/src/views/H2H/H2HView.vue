...
watch(
  () => [props.p1Name, props.p2Name],
  () => updateDocumentTitle(`${p1.value.name} v. ${p2.value.name} | TennisHistory`),
  { immediate: true },
)
...
<template>
  ...
  <a-row v-if="h2h" :gutter="32" class="mb-5">
    <a-col :span="6">
      <a-image
        class="border-2 border-zinc-400 mx-auto rounded-full my-5"
        :src="headshot(h2h.p1[0].id)"
