...
import { EntryConnectionNode, EventDetailsType } from '@/utils/types';
...
const entryInfo: Ref<{ array: EntryConnection['edges'], title: string }[]> = ref([])
...
watch(result, (newResult) => {
    if (newResult) {
        ...
        anchorItems.value = [
            ...
            ...(event.value && event.value.seeds.length > 0 ? [{ key: 'seeds', href: '#seeds', title: 'Seeds' }] : []),
            ...(entryInfo.value.length > 0 ? [{ key: 'info', href: '#entry-info', title: 'Entry Information' }] : []),
