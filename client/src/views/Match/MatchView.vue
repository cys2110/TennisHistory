  ...
  <a-row v-if="match">
    ...
    <a-col :span="20">
      ...
      <a-row class="flex items-center mb-5">
          ...
          <a-card class="h-full flex flex-col justify-between pt-5 items-center">
            <template #cover>
              <a-image
                ...
                :src="headshot(match.p1.player.player.id)"
        ...
        <a-col :span="6">
          <a-config-provider
            :theme="{ components: { Card: { colorBgContainer: COLOURS.green800 } } }"
          >
            <a-card class="h-full flex flex-col justify-between pt-5 items-center">
              <template #cover>
                <a-image
      ...
      <div v-for="stat in stats" :key="stat.category">
        ...
        <StatsChart
          :stats="stat.stats"
          :p1="match.p1.player.player.full_name"
          :p2="match.p2.player.player.full_name"
        />
      </div>
      <div v-if="match.p1.max_speed_kph" id="service-speed" class="text-3xl text-zinc-400">
        SERVICE SPEED
      </div>
