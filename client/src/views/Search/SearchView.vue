...
<template>
  ...
  <a-list
    v-if="results && results.searchPlayers.length > 0"
    :data-source="results.searchPlayers"
    header="Players"
  >
  ...
  <a-list
    v-if="results && results.searchTournaments.length > 0"
    :data-source="results.searchTournaments"
    header="Tournaments"
  >
    <template #renderItem="{ item }">
      <a-list-item>
  ...
  <CountryResults
    v-if="results && results?.searchCountries.length > 0"
    :countries="results.searchCountries"
  />

  <CoachResults
    v-if="results && results?.searchCoaches.length > 0"
    :coaches="results.searchCoaches"
  />

  <UmpireResults
    v-if="results && results?.searchUmpires.length > 0"
    :umpires="results.searchUmpires"
  />

  <SurfaceResults
    v-if="results && results?.searchSurface.length > 0"
    :surfaces="results.searchSurface"
  />

  <VenueResults v-if="results && results?.searchVenue.length > 0" :venues="results.searchVenue" />

  <SupervisorResults
    v-if="results && results?.searchSupervisors.length > 0"
