  ...
  const { records } = await useDriver().executeQuery(
    `
        ...
        WITH y, t, a, collect({
            ...
            t1: CASE
                WHEN w.t1 IS NOT NULL THEN toString(apoc.coll.min([w.t1, l.t1]))
                ELSE NULL
            END,
            t2: CASE
                WHEN w.t2 IS NOT NULL THEN toString(apoc.coll.min([w.t2, l.t2]))
                ELSE NULL
            END,
            t3: CASE
                WHEN w.t3 IS NOT NULL THEN toString(apoc.coll.min([w.t3, l.t3]))
                ELSE NULL
            END,
            t4: CASE
                WHEN w.t4 IS NOT NULL THEN toString(apoc.coll.min([w.t4, l.t4]))
                ELSE NULL
            END,
            t5: CASE
                WHEN w.t5 IS NOT NULL THEN toString(apoc.coll.min([w.t5, l.t5]))
                ELSE NULL
            END
        }) as events
        RETURN {
            website: t.website,
            years: CASE
                WHEN a.id IS NULL THEN toString(y.id) || ' - present'
                WHEN a.id = y.id THEN toString(y.id)
                ELSE toString(y.id) || ' - ' || a.id
            END,
            events: events
        } as tournament
    `,
