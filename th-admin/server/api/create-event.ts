export default defineEventHandler(async event => {
  ...
  const { summary } = await useDriver().executeQuery(
    `/* cypher */
      ...
      e.men_currency = $men_currency, e.women_currency = $women_currency, $e.draw_link = $draw_link, e.pm = CASE WHEN $pm IS NULL THEN NULL ELSE toInteger($pm) END, e.atp_pm = CASE WHEN $atp_pm IS NULL THEN NULL ELSE toInteger($atp_pm) END, e.wta_pm = CASE WHEN $wta_pm IS NULL THEN NULL ELSE toInteger($wta_pm) END,
