            ...
            <v-col cols="3" v-for="(country, countryCode) in teams.groupedWTA" :key="countryCode">
                <v-card class="py-3 px-6" variant="elevated" color="indigo-darken-4" rounded="xl">
                    ...
                    <div v-for="player in teams.groupedATP[countryCode]" class="d-flex my-2 align-center">
                            ...
                            <div>
                                <RouterLink class="hover-link" :to="{name: 'Player', params: {id: player.PlayerId, name: player['Player.first_name'] + ' ' + player['Player.last_name']}}">{{ player['Player.first_name'] }} {{ player['Player.last_name'] }}</RouterLink>
                            </div>
                        </div>
